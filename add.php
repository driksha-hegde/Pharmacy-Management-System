...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
        ...
        if ( $fname && $type && $qty && $company) {
            $query = "INSERT INTO medicines(fname,type,qty,company) VALUES ('{$fname}','$type','$qty','$company')";
        ...
        if ( $fname && $type && $qty && $company ) {
            $query = "UPDATE medicines SET fname='{$fname}', type='{$type}', qty='$qty', company='$company' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allManager" );
        }
